...
class Solution0402 extends Solution
{
#region input
    const INPUT = 
...
byr:1950";
    ...
    public function run()
    {
        ...
        {
            $passport = new Passport2($passportData);
            $valid += $passport->isValid() ? 1 : 0;
        }

        echo "Found $valid valid passports.";
    }
}
...
class Passport2
{
    ...
    public function __construct(string $passportData)
    {
    ...
    public function isValid()
    {
        ...
        if(intval($this->byr) < 1920 || intval($this->byr) > 2002)
        {
            return false;
        }

        if(intval($this->iyr) < 2010 || intval($this->iyr) > 2020)
        {
            return false;
        }

        if(intval($this->eyr) < 2020 || intval($this->eyr) > 2030)
        {
            return false;
        }
        ...
        //ght
        $matches = [];
        if(!preg_match('/^([0-9]+)(in|cm)$/', $this->hgt, $matches))
        {
            return false;
        }
        if($matches[2] === 'cm' && (intval($matches[1]) < 150 || intval($matches[1]) > 193))
        {
            return false;
        }
        if($matches[2] === 'in' && (intval($matches[1]) < 59 || intval($matches[1]) > 76))
        {
            return false;
        }
        ...
        if(!in_array($this->ecl, ['amb', 'blu', 'brn', 'gry', 'grn', 'hzl', 'oth'], true))
        {
            return false;
        }
