...
class Solution0701 extends Solution
{
	public $ready = true;
	...
	public function run()
	{
		...
		{
			$name = explode(' bags contain ', $line)[0];
			$this->rules[$name] = [];
			
			$subBags = explode(', ', explode(' bags contain ', $line)[1]);
			foreach($subBags as $subBag)
			{
				$subCount = intval(substr($subBag,0,strpos($subBag, ' ')));
				$subName = explode(' ', $subBag)[1] . ' ' . explode(' ', $subBag)[2];
				$this->rules[$name][$subName] = $subCount;
			}
		}

		$validBags = [];

		foreach($this->rules as $bagName => $rule)
		{
			if($this->checkBag($bagName, $validBags) && !in_array($bagName, $validBags, true))
			{
				$validBags[] = $bagName;
			}
		}

		//Helper::xout($validBags);
		$validBagsString = implode(', ', $validBags);
		$validBagsCount = count($validBags);
		echo "There are <b>$validBagsCount</b> bags that can hold a shiny gold bag:<br>"; 
		echo "<small>$validBagsString</small>";
	}

	/** @return bool */
	private function checkBag(string $name, array &$validBags, int $level = 0)
	{
		if($level > 32)
		{
		...
		$subBagNames = array_keys($rule);
		$found = false;
		foreach($subBagNames as $subBagName)
		{
			if($subBagName === 'shiny gold')
			{
				$found = true;
			}
			else
			{
				if($this->checkBag($subBagName, $validBags, $level + 1))
				{
					$found = true;
					if(!in_array($subBagName, $validBags, true))
					{
						$validBags[] = $subBagName;
					}
				}
			}
		}
