...
class Solution0602 extends Solution
{
	public $ready = true;
	...
	public function run()
	{
		...
		foreach($groups as $group)
		{
			...
			foreach($persons as $person)
			{
				...
				for($i = 0; $i < $l; $i++)
				{
					$groupAnswers[$person[$i]] = isset($groupAnswers[$person[$i]]) ? $groupAnswers[$person[$i]] + 1 : 1;
				}
			}

			$personCount = count($persons);
			foreach($groupAnswers as $answerCount)
			{
				$total += $answerCount === $personCount ? 1 : 0;
			}
		}

		echo "The total sum is <b>$total</b>";
	}
}