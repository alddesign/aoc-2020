...
class Solution1402 extends Solution
{
	...
	{
		foreach($this->programSets as $programSet)
		{
			foreach($programSet->commands as $command)
			{
				Memory2::write($command->address, $command->value, $programSet->mask);
			}
		}

		return Memory2::getSum();
	}

	private function parseInput()
	{
		/** @var ProgramSet2 $programSet */
		$programSet = null;
		foreach($this::INPUT as $line)
		{
			if(strpos($line, 'mask') === 0)
			{
				$programSet = new ProgramSet2(substr($line, strpos($line, '=') + 2));
				$this->programSets[] = $programSet;
			}
			else
			{
				$address = intval(substr($line, strpos($line, '[') + 1, strpos($line, ']') - strpos($line, '[') - 1));
				$value = intval(substr($line, strpos($line, '=') + 2));
				$programSet->addCommand($address, $value);
			}
		}
	}
}
...
class Memory2
{
	...
	public static function write(string $address, string $inputValue, string $mask)
	{
		$allAddresses = self::calcAddresses($address, $mask);
		foreach($allAddresses as $addr)
		{
			self::$memory[$addr] = $inputValue;
		}
	}

	private static function calcAddresses(string $address, string $mask)
	{
		$allAddresses = [];
		...
		for($i = 0; $i < 36; $i++)
		{
			if($mask[$i] === 'X')
			{
				$floatingBitPositions[] = $i;
				$address[$i] = 'X';
			}
			else
			{
				$address[$i] = $mask[$i] === '1' ? '1' : $address[$i]; 
			}
		}

		$l = count($floatingBitPositions);
		$l1 = pow(2, $l);
		for($i = 0; $i < $l1; $i++)
		{
			$newAddress = $address;
			$floats = str_pad(decbin($i), $l, '0', STR_PAD_LEFT);
			foreach($floatingBitPositions as $fi => $pos)
			{
				$newAddress[$pos] = $floats[$fi];
			}
...
class ProgramSet2
{
	...
	public function addCommand(int $address, int $value)
	{
		$this->commands[] = new Command2($address, $value);
	}
}
...
class Command2
{
	...
	{
		$this->address = str_pad(decbin($address), 36, '0', STR_PAD_LEFT);
		$this->value = str_pad(decbin($value), 36, '0', STR_PAD_LEFT);
	}
}
