...
class Solution0702 extends Solution
{
	public $ready = true;
	...
	public function run()
	{
		...
		$total = 0;
		$this->digDown('shiny gold', 1, $total);

		echo "A shiny gold bag must contain <b>$total</b> other bags.";
	}

	/** @return bool */
	private function digDown(string $name, int $amount, int &$total, int $level = 0)
	{
		if($level > 32)
		{
		...
		foreach($rule as $subBagName => $subAmount)
		{
			$total += $amount * $subAmount;
			$this->digDown($subBagName, $amount * $subAmount, $total, $level + 1);
		}
	}
}