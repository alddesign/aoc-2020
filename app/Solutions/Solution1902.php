...
class Solution1902 extends Solution
{
	public $ready = true;
	...
	public function run()
	{
		...
		foreach($this::RULES as $rule)
		{
			...
			if($i !== false)
			{
				...
				$this->rules[$id] = Rule4::makeLiteral($id, $char);
			}
			else
			{
				$subs = substr($rule, strpos($rule, ':') + 2);
				$this->rules[$id] = Rule4::makeSub($id, $subs);
			}
		}
		ksort($this->rules);

		//Get the whole thing as a regex:
		$regex = RuleHelper2::getRegex($this->rules[0], $this->rules);
 ...
 * Credits to jingfee: https://github.com/jingfee/advent-of-code-rust/blob/main/src/y2020/day19.rs 
 */
class RuleHelper2
{
	public static function getRegex(Rule4 $rule, array &$rules, bool $init = true, array $path = [])
	{
		$regex = '';
		if($rule->literal)
		{
		...
		else
		{
			...
			foreach($rule->subs as $sgid => $subGroup)
			{
				...
				foreach($subGroup as $id)
				{
					if($id === 8)
					{
						$regex .= RuleHelper2::getRegex($rules[42], $rules, false, $path) . '+';
					}
					elseif($id === 11)
					{
						$regex .= '('.RuleHelper2::getRegex($rules[42], $rules, false, $path).'(?1)?'.RuleHelper2::getRegex($rules[31], $rules, false, $path).')';
					}
					else
					{
						$regex  .= RuleHelper2::getRegex($rules[$id], $rules, false, $path);
					}
				}
			}
			$regex  .= ')';
