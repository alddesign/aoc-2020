...
class Solution1401 extends Solution
{
	...
	private function runProgram()
	{
		foreach($this->programSets as $programSet)
		{
			...
			{
				Memory::write($command->address, $command->value, $programSet->mask);
			}
		}
	...
	{
		/** @var ProgramSet $programSet */
		$programSet = null;
		foreach($this::INPUT as $line)
		{
			if(strpos($line, 'mask') === 0)
			{
				$programSet = new ProgramSet(substr($line, strpos($line, '=') + 2));
...
class Memory
{
	...
	public static function write(int $address, string $inputValue, string $mask)
	{
		$value = '';
		...
		for($i = 0; $i < 36; $i++)
		{
			switch($mask[$i])
			{
				...
				case('X') : $value .= $inputValue[$i]; break;
			}
		}
...
class ProgramSet
{
	...
	{
		$this->commands[] = new Command($address, $value);
	}
}
