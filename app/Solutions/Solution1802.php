...
class Solution1802 extends Solution
{
	...
	//Test
	private const INPUT1 = ['((2 + 4 * 9) * (6 + 9 * 8 + 6) + 6) + 2 + 4 * 2'];
	...
	public function run()
	{
		...
		echo "The result is <b>$total</b>";
	}

	/** @return int */
	private function calculate(string $line, int &$i = 0)
	{
		$line2 = '';
		$l = strlen($line);
		...
		//Now only multiplications:
		$multiply = explode('*', $line2);
		$result = 1;
		foreach($multiply as $m)
		{
			$result *= intval($m);
		}
