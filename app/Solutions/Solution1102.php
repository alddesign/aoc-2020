...
class Solution1102 extends Solution
{
	...
	public function run()
	{
		...
		$finalSeatsOccupied = $this->calculate();
		$duration = round(microtime(true) - $start, 3);

		echo "<div>In the end there are <b>$finalSeatsOccupied</b> occupied seats. <small>Execution duration: $duration sec.</small><div>";
		$this->echoJsonData();
	}

	private function echoJsonData()
	{
		echo '<script> var maps = '. json_encode($this->allMaps) .'; </script>' . "\n";
		echo '<script src="'.Helper::url('assets/Solution11.js').'"></script>'. "\n";
	}
	...
	private function calculate()
	{
		...
		while(true)
		{
			...
			for($r = 0; $r < $rows; $r++)
			{
				for($c = 0; $c < $rowLen; $c++)
				{
					if($this->map[$r][$c] === 'L' && $this->countVisibleSeatsOccupied($r, $c) === 0)
					{
						$newMap[$r][$c] = '#';
					}
					elseif($this->map[$r][$c] === '#' && $this->countVisibleSeatsOccupied($r, $c) >= 5)
					{
						$newMap[$r][$c] = 'L';
					}
					else
			...
			if($this->map == $newMap)
			{
				...
				for($r = 0; $r < $rows; $r++)
				{
					for($c = 0; $c < $rowLen; $c++)
					{
						$finalSeatsOccupied += $this->map[$r][$c] === '#' ? 1 : 0;
					}
				}

				return $finalSeatsOccupied;
			}
	...
	/** @return int */
	private function countVisibleSeatsOccupied(int $r, int $c)
	{
		$seatsOccupied = 0;

		$seatsOccupied += $this->countDirection($r, $c, -1, -1); //above-left
		$seatsOccupied += $this->countDirection($r, $c,  0, -1); //above
		$seatsOccupied += $this->countDirection($r, $c, +1, -1); //above-right
		
		$seatsOccupied += $this->countDirection($r, $c, -1, 0); //left
		$seatsOccupied += $this->countDirection($r, $c, +1, 0); //right

		$seatsOccupied += $this->countDirection($r, $c, -1, +1); //below-left
		$seatsOccupied += $this->countDirection($r, $c,  0, +1); //below
		$seatsOccupied += $this->countDirection($r, $c, +1, +1); //below-right

		return $seatsOccupied;
	}

	/** @return int */
	private function countDirection(int $r, int $c, int $xf, int $yf)
	{
		$i = 1;
		while(true)
		{
			if(isset($this->map[$r + $i * $xf][$c + $i *$yf]))
			{
				if($this->map[$r + $i * $xf][$c + $i *$yf] === '#')
				{
					return 1;
				}
				if($this->map[$r + $i * $xf][$c + $i *$yf] === 'L')
				{
					return 0;
				}
			}
