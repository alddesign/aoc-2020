...
class Solution2101 extends Solution
{
	public $ready = true;
	...
	/** @var Food[] $foods */
	private $foodsUntouched = [];
	/** @var Food[] $foods */
	private $foods = [];
	/** @var string[] $allergenes */
	private $allergenes = [];
	...
	public function run()
	{
		...
		//Create objects n shit
		$this->parseInput();

		//repeat until there are no more allergenes and some leftover ingredients
		while(!empty($this->allergenes))
		{
			$this->processFood();
		}

		//count the leftovers
		$leftoverIngredients = $this->ingredients;
		$total = 0;
		foreach($this->foodsUntouched as $food)
		{
			foreach($leftoverIngredients as $ingredient)
			{
				$total += $food->hasIngredient($ingredient) ? 1 : 0;
			}
		}

		echo "Total is <b>$total<b>";
	}
	...
	private function parseInput()
	{
		...
		{
			$ingredients = explode(' ', substr($data, 0, strpos($data, '(') - 1));
			$p = strpos($data, '(') + 10;
			$l = strlen($data) - $p - 1;
			$allergenes = explode(', ', substr($data, $p, $l));

			$this->foods[] = new Food($ingredients, $allergenes);
			$this->foodsUntouched[] = new Food($ingredients, $allergenes); //We need to keep a clean copy!

			foreach($ingredients as $i)
			{
				if(!in_array($i, $this->ingredients,true)){$this->ingredients[$i] = $i;}
			}

			foreach($allergenes as $a)
			{
				if(!in_array($a, $this->allergenes,true)){$this->allergenes[$a] = $a;}
			}
		}
	}
	private function processFood()
	{
		foreach($this->foods as $food)
		{
			foreach($food->ingredients as $ingredient)
			{
				foreach($food->allergenes as $allergene)
				{
					$matches = $this->checkIngredientAllergeneCombo($ingredient, $allergene);
					if($matches > 0)
					{
						//Check if there is an allergene which has a higher "rating" (more foods where this logic applies)
						//Ex: if there are 14 foods which contain peanuts and "xadvx", but 16 foods which contain eggs and "xadvx" --> eggs is there more correct one
						$higerRatedAllergene = '';
						$crossMatches = $this->checkCrossMatches($ingredient, $allergene, $higerRatedAllergene);
						if($matches > $crossMatches)
						{
							unset($this->ingredients[$ingredient]);
							unset($this->allergenes[$allergene]);
							$this->removeFromAllFoods($ingredient, $allergene);
		...
		throw new Exception('Unable to relove that shit!');
	}

	//Check if there is another allergene with a higher matching rate:
	private function checkCrossMatches(string $targetIngredient, string $originalAllergene, string &$higerRatedAllergene)
	{
		$max = 0;
		$higerRatedAllergene = '';
		foreach($this->allergenes as $allergene)
		{
			if($allergene !== $originalAllergene)
			{
				$matches = $this->checkIngredientAllergeneCombo($targetIngredient, $allergene);
				if($matches > $max)
				{
					$max = $matches;
					$higerRatedAllergene = $allergene;
				}
			}
		}
	...
	private function checkIngredientAllergeneCombo(string $ingredient, string $allergene)
	{
		//Assume: ingredient = x and allergene = y.
		//Every food that has allergene y must have ingredient x. Otherwise ingredient x doesnt have allergene y.
		$found = 0;
		foreach($this->foods as $food)
		{
			if($food->hasAllergene($allergene))
			{
	...
	private function removeFromAllFoods(string $ingredient, string $allergene)
	{
		foreach($this->foods as $food)
		{
			if(isset($food->ingredients[$ingredient])){ unset($food->ingredients[$ingredient]); }
			if(isset($food->allergenes[$allergene])){ unset($food->allergenes[$allergene]); }
		}
	}
}
...
class Food
{
	...
	public function __construct(array $ingredients, array $allergenes)
	{
		foreach($allergenes as $a)
		{
			$this->allergenes[$a] = $a;
	...
	public function hasAllergene(string $allergene)
	{
		return isset($this->allergenes[$allergene]);
	}

	public function hasIngredient(string $ingredient)
	{
		return isset($this->ingredients[$ingredient]);
	}

	public function hasIngredients(array $ingredients)
	{
		foreach($ingredients as $i)
		{
			if(!isset($this->ingredients[$i]))
			{
				return false;
			}
		}
